...
var depcheck = require('check-dependencies');
...
var checkDependencies = function checkDependencies(callback) {
    depcheck().then(function(result){
        if (result.status === 0) {
            return callback(null, result);
        }
        return callback(new Error('build-required', result));
    });
};
...
var deployProject = function(directory, done) {
    ...
    async.waterfall([
        ...
        waterfallChain(checkDependencies),
    ...
    function (error, results) {
        if (error) {
            ...
            } else if (error.message === 'build-required') {
                console.log(
                    chalk.red('\n\r Run `npm install` in order to deploy')
                );
